  ...
  <v-dialog v-model="isValidForm" max-width="500">
    <v-card v-if="isValidForm" id="featureXMLForm">
      <v-card-title class="d-flex justify-space-between align-center">
        <div class="text-medium-emphasis ps-2 title">
          {{ formConfig.formFields.form.title }}
        </div>
      ...
      <v-card-text>
        <v-form ref="featureOption" fast-fail v-model="isValidForm">
          <div
            v-for="field in formConfig.formFields.form.fields"
            :key="field.name"
          >
      ...
      <v-card-actions>
        ...
        <v-btn
          ...
          @click="submitForm"
          >{{ formConfig.buttons.OK }}</v-btn
        >
        <v-btn @click="closeForm" color="#e01b3c">{{
          formConfig.buttons.Cancel
        }}</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
